...
public class MpvPlugin implements Plugin {

    private static final List<Datavyu.Platform> validOperatingSystem = Lists.newArrayList(Datavyu.Platform.WINDOWS);

    private static final UUID pluginUUID = UUID.nameUUIDFromBytes("plugin.mpv".getBytes());
    ...
    public Class<? extends StreamViewer> getViewerClass() {
        return Platform.isWindows() ? MpvStreamViewer.class : null;
    }
